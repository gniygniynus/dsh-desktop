...
function resolveTarget(relPath) {
  ...
  const homes = [homedir(), process.env.USERPROFILE, "/mnt/c/Users/Lenovo"];
...
function applyWorkspaceDeletePatch() {
  ...
  const replacements = [
    ...
    ["archiveSession: async (sessionId) => {\n\t\t\t\t\tawait ctx.workspaces.archiveSession(sessionId);\n\t\t\t\t},",
     "archiveSession: async (sessionId) => {\n\t\t\t\t\tawait ctx.workspaces.archiveSession(sessionId);\n\t\t\t\t},\n\t\t\t\tdeleteSession: (sessionId) => {\n\t\t\t\t\tconsole.log('[dsh-session-rewind] delete 调用', sessionId);\n\t\t\t\t\treturn ctx.remote.sessionRewind.delete({ sessionId }).then((r) => {\n\t\t\t\t\t\tconsole.log('[dsh-session-rewind] delete 结果', JSON.stringify(r));\n\t\t\t\t\t\tif (r.ok && r.value && r.value.ok) { console.log('[dsh-session-rewind] refresh 调用'); ctx.sessions.refresh().then(() => console.log('[dsh-session-rewind] refresh 完成')).catch((e) => console.error('[dsh-session-rewind] refresh 错误', e)); }\n\t\t\t\t\t}).catch((e) => {\n\t\t\t\t\t\tconsole.error('[dsh-session-rewind] delete 错误', e);\n\t\t\t\t\t});\n\t\t\t\t},"],
...
function applyConversationRewindPatch() {
  ...
  const replacements = [
    ...
    ["\t\t\t\t\t\tforkAt: (seq) => {\n\t\t\t\t\t\t\tsessions.fork({\n\t\t\t\t\t\t\t\tsessionId,\n\t\t\t\t\t\t\t\tatSeq: seq,\n\t\t\t\t\t\t\t\tincreaseTitle: true\n\t\t\t\t\t\t\t}).then((childId) => {\n\t\t\t\t\t\t\t\tsessions.open(childId);\n\t\t\t\t\t\t\t}).catch(() => {});\n\t\t\t\t\t\t}",
     "\t\t\t\t\t\tforkAt: (seq) => {\n\t\t\t\t\t\t\tsessions.fork({\n\t\t\t\t\t\t\t\tsessionId,\n\t\t\t\t\t\t\t\tatSeq: seq,\n\t\t\t\t\t\t\t\tincreaseTitle: true\n\t\t\t\t\t\t\t}).then((childId) => {\n\t\t\t\t\t\t\t\tsessions.open(childId);\n\t\t\t\t\t\t\t}).catch(() => {});\n\t\t\t\t\t\t},\n\t\t\t\t\t\trewindAt: (seq) => {\n\t\t\t\t\t\t\tconsole.log('[dsh-session-rewind] rewind 调用', sessionId, seq);\n\t\t\t\t\t\t\tctx.remote.sessionRewind.rewind({\n\t\t\t\t\t\t\t\tsessionId,\n\t\t\t\t\t\t\t\tatSeq: seq\n\t\t\t\t\t\t\t}).then((carried) => {\n\t\t\t\t\t\t\t\tconsole.log('[dsh-session-rewind] rewind 结果', JSON.stringify(carried));\n\t\t\t\t\t\t\t\tif (carried.ok && carried.value && carried.value.ok) { const childId = carried.value.value.sessionId; if (childId) ctx.sessions.refresh().then(() => sessions.open(childId)).catch((e) => console.error('[dsh-session-rewind] open 失败', e)); else ctx.sessions.refresh(); }\n\t\t\t\t\t\t\t}).catch((e) => {\n\t\t\t\t\t\t\t\tconsole.error('[dsh-session-rewind] rewind 错误', e);\n\t\t\t\t\t\t\t});\n\t\t\t\t\t\t},\n\t\t\t\t\t\tregenerateAt: (seq) => {\n\t\t\t\t\t\t\tconsole.log('[dsh-session-rewind] regenerate 调用', sessionId, seq);\n\t\t\t\t\t\t\tctx.remote.sessionRewind.regenerate({ sessionId, atSeq: seq }).then((carried) => {\n\t\t\t\t\t\t\t\tconsole.log('[dsh-session-rewind] regenerate 结果', JSON.stringify(carried));\n\t\t\t\t\t\t\t\tif (carried.ok && carried.value && carried.value.ok) {\n\t\t\t\t\t\t\t\t\tconst childId = carried.value.value.sessionId;\n\t\t\t\t\t\t\t\t\tif (childId) ctx.sessions.refresh().then(() => sessions.open(childId)).catch((e) => console.error('[dsh-session-rewind] open 失败', e));\n\t\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t}).catch((e) => {\n\t\t\t\t\t\t\t\tconsole.error('[dsh-session-rewind] regenerate 错误', e);\n\t\t\t\t\t\t\t});\n\t\t\t\t\t\t}"],

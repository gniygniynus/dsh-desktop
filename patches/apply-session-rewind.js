...
function applyConversationRewindPatch() {
  ...
  const replacements = [
    ...
    ["\t\t\t\t\t\tforkAt: (seq) => {\n\t\t\t\t\t\t\tsessions.fork({\n\t\t\t\t\t\t\t\tsessionId,\n\t\t\t\t\t\t\t\tatSeq: seq,\n\t\t\t\t\t\t\t\tincreaseTitle: true\n\t\t\t\t\t\t\t}).then((childId) => {\n\t\t\t\t\t\t\t\tsessions.open(childId);\n\t\t\t\t\t\t\t}).catch(() => {});\n\t\t\t\t\t\t}",
     "\t\t\t\t\t\tforkAt: (seq) => {\n\t\t\t\t\t\t\tsessions.fork({\n\t\t\t\t\t\t\t\tsessionId,\n\t\t\t\t\t\t\t\tatSeq: seq,\n\t\t\t\t\t\t\t\tincreaseTitle: true\n\t\t\t\t\t\t\t}).then((childId) => {\n\t\t\t\t\t\t\t\tsessions.open(childId);\n\t\t\t\t\t\t\t}).catch(() => {});\n\t\t\t\t\t\t},\n\t\t\t\t\t\trewindAt: (seq) => {\n\t\t\t\t\t\t\tctx.remote.sessionRewind.rewind({\n\t\t\t\t\t\t\t\tsessionId,\n\t\t\t\t\t\t\t\tatSeq: seq\n\t\t\t\t\t\t\t}).then((r) => {\n\t\t\t\t\t\t\t\t// typert 网关返回单层业务结果 {ok, value}\n\t\t\t\t\t\t\t\tif (r.ok) { const childId = r.value.sessionId; if (childId) ctx.sessions.refresh().then(() => sessions.open(childId)).catch(() => {}); else ctx.sessions.refresh().catch(() => {}); }\n\t\t\t\t\t\t\t\telse { const code = r.error?.code ?? 'unknown'; alert('撤回失败：' + code); }\n\t\t\t\t\t\t\t}).catch((e) => {\n\t\t\t\t\t\t\t\talert('撤回出错：' + String(e));\n\t\t\t\t\t\t\t});\n\t\t\t\t\t\t},\n\t\t\t\t\t\tregenerateAt: (seq) => {\n\t\t\t\t\t\t\tctx.remote.sessionRewind.regenerate({ sessionId, atSeq: seq }).then((r) => {\n\t\t\t\t\t\t\t\t// typert 网关返回单层业务结果 {ok, value}\n\t\t\t\t\t\t\t\tif (r.ok) {\n\t\t\t\t\t\t\t\t\tconst childId = r.value.sessionId;\n\t\t\t\t\t\t\t\t\tif (childId) ctx.sessions.refresh().then(() => sessions.open(childId)).catch(() => {});\n\t\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t\telse { const code = r.error?.code ?? 'unknown'; alert('重新回答失败：' + code); }\n\t\t\t\t\t\t\t}).catch((e) => {\n\t\t\t\t\t\t\t\talert('重新回答出错：' + String(e));\n\t\t\t\t\t\t\t});\n\t\t\t\t\t\t}"],
